...
| 25 | Agent Control Centre | `/brandscope-admin/agents` | Internal admin | `agents`, `agent_skills`, `agent_job_logs`, `prompt_versions`, `model_router_config` (+ generated manifest) | AgentsRoster (kill switches + module switches + skill toggles), AgentConfigPanel (Declared/Observed + editable model/temperature/max-tokens), AgentPromptEditor (versioned prompts, warn-don't-block activation, rollback), AgentSandbox (red-team presets), AgentsPromptVersions, AgentsJobTrace |
