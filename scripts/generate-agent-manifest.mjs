...
for (const fn of fnDirs) {
  ...
  const routerTasks = [
    ...src.matchAll(/resolveModel\(\s*sb,\s*"([a-z_]+)",\s*MODELS\.(\w+)\s*\)/g),
    ...src.matchAll(/resolveRoute\(\s*sb,\s*"([a-z_]+)",\s*\{\s*\n?\s*model: MODELS\.(\w+)/g),
  ].map((m) => ({ task: m[1], codeDefault: m[2] }));
...
// ── prompt slot templates (exported backtick constants; P2c editable prompts) ─
const SLOT_CONST = {
  "researcher-promotions": { PROMOTIONS_SYSTEM: "researcher:promotions" },
  "researcher-customer": { CUSTOMER_SYSTEM: "researcher:customer" },
  "researcher-regulatory": { REGULATORY_SYSTEM: "researcher:regulatory" },
  "researcher-hiring": { HIRING_SYSTEM: "researcher:hiring" },
  "researcher-traffic-seo": { TRAFFIC_SYSTEM: "researcher:traffic_seo" },
  "researcher-geo-aeo": { GEO_SYSTEM_TEMPLATE: "researcher:geo_aeo" },
  "synthesis-draft-audit": {
    SUPERVISOR_SYSTEM: "supervisor",
    DRAFTER_SYSTEM_TEMPLATE: "drafter",
    AUDITOR_SYSTEM: "auditor",
  },
};
const promptSlots = {};
for (const [fn, consts] of Object.entries(SLOT_CONST)) {
  const src2 = readDirSources(fn);
  for (const [constName, slot] of Object.entries(consts)) {
    const m = src2.match(new RegExp("export const " + constName + " = `([\\s\\S]*?)`;"));
    if (!m) throw new Error(`template const ${constName} not found in ${fn} — extraction drift?`);
    promptSlots[slot] = { template: m[1], constName, fn };
  }
}
if (Object.keys(promptSlots).length !== 9) {
  throw new Error(`expected 9 prompt slots, got ${Object.keys(promptSlots).length}`);
}
